...
if __name__ == '__main__':
    users = os.environ["wzxy_wq"].split('&')
    ...
    for user in users:
        try:
            ...
            pushPlusToken = user[3].split('=')[1]
            onlyWrongNotify = user[4].split('=')[1]
            mark = user[5].split('=')[1]
            configs = {"wozaixiaoyuan_data": {"username": username, "password": password, "location": location},
                       "pushPlus_data": {"notifyToken": pushPlusToken, "onlyWrongNotify": onlyWrongNotify},
                       "mark": mark}
